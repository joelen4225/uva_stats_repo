...
feats = ['release_speed', 'release_pos_x', 'release_pos_z', 'is_lhp', 'is_lhb', 'balls', 
         'strikes', 'pfx_x', 'pfx_z', 'plate_x', 'plate_z', 'on_3b', 'on_2b', 'on_1b', 
         'outs_when_up', 'inning', 'is_top', 'vx0', 'vy0', 'vz0', 'ax', 'ay', 'az', 
         'sz_top', 'sz_bot', 'effective_speed', 'release_spin_rate', 'release_extension', 
         'release_pos_y', 'pitch_number', 'spin_axis', 'spray_angle', 'bat_speed', 
         'swing_length', 'pitch_type_CH', 'pitch_type_CU', 'pitch_type_EP', 'pitch_type_FA',
...
# Using Only Significant Features for Modeling (Feature Importances)
## Finding and Pointing Out Important Values
model = RandomForestClassifier(n_estimators=100, random_state=42) 
model.fit(train_df[feats], train_df[target])
feature_importances = model.feature_importances_
importance_df = pd.DataFrame({'Feature': feats, 'Importance': feature_importances})
importance_df = importance_df.sort_values(by='Importance', ascending=False)
print('---------------------------------------------------')
print(importance_df)
## Setting a Threshold of 0.03 for Importance
threshold = 0.03
...
rfe = rfe.fit(train_df[feats], train_df[target])
